    ...
    <template v-else>
      ...
      <div class="row gy-4">
        ...
        <div class="col-xl-4">
          ...
          <section class="detail-card offers-card mb-24">
            <div class="section-heading"><span><iconify-icon icon="lucide:hand-coins" /></span><h6 class="mb-0">Offers Received</h6></div>
            <div v-if="offersLoading" class="offers-state">Loading offers...</div>
            <div v-else-if="offersError" class="offers-state offers-state--error">{{ offersError }}</div>
            <div v-else-if="receivedOffers.length" class="offer-list">
              <div v-for="offer in receivedOffers" :key="offer.id" class="offer-item">
                <div class="offer-item__top">
                  <strong>{{ formatCurrency(offer.amount) }}</strong>
                  <PropertyStatusBadge :status="offer.status" />
                </div>
                <div class="offer-item__person">
                  <span>From</span>
                  <strong>{{ personName(offer.sender) }}</strong>
                  <small>{{ safeValue(offer.sender?.email || offer.sender?.phone) }}</small>
                </div>
                <div class="offer-item__meta">
                  <span>#{{ offer.id }}</span>
                  <span>{{ formatDate(offer.created_at, true) }}</span>
                  <span :class="offer.is_seen_by_vendor ? 'seen' : 'unseen'">{{ offer.is_seen_by_vendor ? 'Seen' : 'Unseen' }}</span>
                </div>
              </div>
            </div>
            <div v-else class="offers-state">No offers received for this property.</div>
            <router-link :to="`/offers?post_id=${property.id}&direction=received`" class="offers-link"><iconify-icon icon="lucide:external-link" /> View all offers</router-link>
          </section>
...
const route=useRoute(),router=useRouter(),auth=useAuthStore();const featureEnabled=features.propertyFeatured;const property=ref({}),receivedOffers=ref([]),loading=ref(true),offersLoading=ref(false),error=ref(''),offersError=ref(''),busy=ref(false)
...
function personName(person){return person?.name||person?.agency_name||person?.email||person?.phone||(person?.id?`User #${person.id}`:'Not available')}
function normalizeOffers(value){return Array.isArray(value)?value:value?.data||[]}
async function loadOffers(){offersLoading.value=true;offersError.value='';try{receivedOffers.value=normalizeOffers(await offerService.getOffers({post_id:route.params.id,direction:'received',per_page:10,sort_by:'created_at',sort_direction:'desc'}))}catch(err){offersError.value=err.message;receivedOffers.value=[]}finally{offersLoading.value=false}}
async function load(){loading.value=true;error.value='';try{property.value=await propertyService.getProperty(route.params.id)||{};await loadOffers()}catch(err){error.value=err.message}finally{loading.value=false}}
